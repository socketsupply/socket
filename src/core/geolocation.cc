...
namespace SSC {
  Geolocation::Geolocation (Core* core)
    : Module(core),
  ...
  void Geolocation::getCurrentPosition (const String& seq, const Module::Callback callback) {
  ...
  void Geolocation::watchPosition (const String& seq, uint64_t id, const Module::Callback callback) {
  #if SSC_PLATFORM_APPLE
  ...
  template<> bool Module::template Observers<Module::template Observer<JSON::Object>>::add(
    const Module::Observer<JSON::Object>&,
    Module::Observer<JSON::Object>::Callback
  );

  template<> bool Geolocation::PermissionChangeObservers::remove(
    const Geolocation::PermissionChangeObserver&
  );
