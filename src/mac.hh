...
- (void) draggingExited: (id<NSDraggingInfo>)info {
  auto payload = op::emitToRenderProcess("dragend", "{}");
...
- (NSDragOperation) draggingUpdated:(id <NSDraggingInfo>)info {
  ...
  auto payload = op::emitToRenderProcess("drag", json);
...
- (void) draggingEnded: (id<NSDraggingInfo>)info {
  ...
  if (NSPointInRect([info draggingLocation], self.frame)) {
    ...
    for (NSURL *url in files) {
      ...
      auto payload = op::emitToRenderProcess("dropin", json);
...
- (void) mouseDown: (NSEvent*)event {
  ...
  [self
    ...
    std::vector<std::string> files =
      op::split(std::string([result UTF8String]), ';');
  ...
  completionHandler:(void (^)(NSError *errorOrNil))completionHandler
{
  ...
  std::string js = op::emitToRenderProcess("dropout", json);
...
- (NSString*) filePromiseProvider: (NSFilePromiseProvider*)filePromiseProvider fileNameForType:(NSString *)fileType {
  std::string file(std::to_string(op::rand64()) + ".download");
